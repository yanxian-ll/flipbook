...
export function VisualTemplateEditor({slots,onChange,assetIds,background,overlay,texts=[]}:{slots:Slot[];onChange:(slots:Slot[])=>void;assetIds:string[];background:string;overlay?:string;texts?:Element[]}){
  ...
  function move(e:ReactPointerEvent<HTMLElement>){const g=gesture.current;if(!g||g.pointerId!==e.pointerId)return;onChange(g.original.map((slot,index)=>index===g.index?resizeSlot(slot,g.handle,(e.clientX-g.x)/g.width,(e.clientY-g.y)/g.height):slot));}
