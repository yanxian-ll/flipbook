...
const decorationEdgeStyle=(edge:'top'|'right'|'bottom'|'left'):CSSProperties=>({position:'absolute',padding:0,background:'transparent',pointerEvents:'auto',zIndex:2,...(edge==='top'?{left:0,right:0,top:-5,height:10,cursor:'move'}:edge==='right'?{top:0,bottom:0,right:-5,width:10,cursor:'move'}:edge==='bottom'?{left:0,right:0,bottom:-5,height:10,cursor:'move'}:{top:0,bottom:0,left:-5,width:10,cursor:'move'})});
...
export function VisualTemplateEditor({slots,onChange,assetIds,background,overlay,texts=[],maxSlots=9}:{slots:Slot[];onChange:(slots:Slot[])=>void;assetIds:string[];background:string;overlay?:string;texts?:Element[];maxSlots?:number}){
  ...
  return <div className="visual-template-editor">
    ...
    <div className="template-canvas" ref={canvas} style={{background}} aria-label="可视化模板画布">
      ...
      {decorations.map((decoration,index)=><div key={decoration.id} className={`template-frame template-border-decoration ${selectedDecoration===index?'selected':''}`} style={{left:`${decoration.x*100}%`,top:`${decoration.y*100}%`,width:`${decoration.width*100}%`,height:`${decoration.height*100}%`,zIndex:4,pointerEvents:'none'}}><div aria-hidden="true" style={{position:'absolute',inset:0,boxShadow:`inset 0 0 0 max(1px, ${(decoration.strokeWidth||1)/12}cqw) ${decoration.stroke||'#111'}`,pointerEvents:'none'}}/>{(['top','right','bottom','left'] as const).map(edge=><button key={edge} type="button" aria-label={`装饰边框 ${index+1}，拖动移动`} onFocus={()=>setSelectedDecoration(index)} onPointerDown={e=>begin(e,'decoration',index,'move')} onPointerMove={move} onPointerUp={end} onPointerCancel={cancel} style={decorationEdgeStyle(edge)}/>)}{selectedDecoration===index&&<span aria-hidden="true" style={{position:'absolute',left:4,top:4,fontSize:9,background:'#3185ff',color:'#fff',padding:'1px 4px',borderRadius:3,pointerEvents:'none'}}>边框</span>}{selectedDecoration===index&&(['nw','n','ne','e','se','s','sw','w'] as Handle[]).map(handle=><button key={handle} type="button" className={`template-resize-handle handle-${handle}`} aria-label={`装饰边框 ${index+1} ${handle} 调整大小`} onPointerDown={e=>begin(e,'decoration',index,handle)} onPointerMove={move} onPointerUp={end} onPointerCancel={cancel} style={{pointerEvents:'auto'}}/>)}</div>)}
