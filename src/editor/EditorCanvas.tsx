...
import {Stage,Layer,Rect,Text,Image as CanvasImage,Transformer,Line,Group,Path,Circle} from 'react-konva';
...
function rotationHandlePosition(element:Element,offset:number){
  const angle=(element.rotation??0)*Math.PI/180;
  const localX=element.width/2,localY=-offset;
  return {
    x:element.x+localX*Math.cos(angle)-localY*Math.sin(angle),
    y:element.y+localX*Math.sin(angle)+localY*Math.cos(angle),
  };
}
