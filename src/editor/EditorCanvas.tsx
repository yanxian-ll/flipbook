...
function MoveHandle({element,scale,onMove,onCommit}:{element:Element;scale:number;onMove:(dx:number,dy:number)=>void;onCommit:()=>void}){
  const safeScale=Math.max(.1,scale),w=44/safeScale,h=27/safeScale,gap=7/safeScale,pad=4/safeScale;
  const x=Math.max(pad,Math.min(W-w-pad,element.x+element.width/2-w/2));
