...
import {applyLayout,layouts} from '../domain/layouts';
...
interface EditorState {
  ...
  layout:(id:string)=>void; addAssets:(assets:StoredAsset[])=>Promise<void>; flush:()=>Promise<void>;
}
...
export const useEditor=create<EditorState>((set,get)=>({
  ...
  updateElement(id,patch){get().change(b=>{const e=b.pages[get().pageIndex]?.elements.find(e=>e.id===id);if(e)Object.assign(e,patch);});},
  addElement(element){get().change(b=>{b.pages[get().pageIndex].elements.push(element);});set({selected:[element.id]});},
  deleteSelected(){const selected=get().selected;if(!selected.length)return;get().change(b=>{const page=b.pages[get().pageIndex];page.elements=page.elements.filter(e=>!selected.includes(e.id)||e.locked);});set({selected:[]});},
  copy(){const s=get();set({clipboard:structuredClone(s.book?.pages[s.pageIndex].elements.filter(e=>s.selected.includes(e.id))??[])});},
  paste(){const items=get().clipboard.map(e=>({...e,id:uid(),x:e.x+30,y:e.y+30}));if(!items.length)return;get().change(b=>{b.pages[get().pageIndex].elements.push(...items);});set({selected:items.map(e=>e.id)});},
  ...
  layout(id){const layout=layouts.find(l=>l.id===id);if(!layout)return;get().change(b=>{b.pages[get().pageIndex]=applyLayout(b.pages[get().pageIndex],layout);});set({selected:[]});},
