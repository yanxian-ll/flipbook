...
export function Editor(){
  ...
  function turnPages(direction:'next'|'prev',pageIndex=s.pageIndex){
  ...
  function canPhysicalTurn(direction:'next'|'prev'){
    if(coverView||coverPhase!=='idle'||!turnPages(direction))return false;
  ...
  function turnPage(direction:'next'|'prev',after?:()=>void){
    const state=useEditor.getState(),currentIndex=state.pageIndex,turn=turnPages(direction,currentIndex);
    if(turnLocked.current||!turn||turn.target===currentIndex)return;
    turnLocked.current=true;setTurnDirection(direction);
    const originY=.5,target=turn.target;
    requestAnimationFrame(()=>requestAnimationFrame(()=>{applyTurnVisual(0,direction,originY);requestAnimationFrame(()=>{applyTurnVisual(1,direction,originY,'transform 310ms cubic-bezier(.18,.72,.18,1)');if(turnTimer.current!==null)window.clearTimeout(turnTimer.current);turnTimer.current=window.setTimeout(()=>{const latest=useEditor.getState();if(target!==latest.pageIndex)latest.setPage(target);clearTurn();after?.();},328);});}));
  }
