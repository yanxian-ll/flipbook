...
export function Editor(){
  ...
  const workspace=useRef<HTMLDivElement>(null),replaceInput=useRef<HTMLInputElement>(null),flipBook=useRef<EditorFlipBookHandle>(null),focusTrack=useRef<HTMLDivElement>(null),suppressSpreadClick=useRef(false);
        ...
        {zoomMode==='spread'
          ...
          :<div className="focused-book-shell single-page-editor-shell" style={{width:focusWindowWidth,height:pageHeight}}>
            <div ref={focusTrack} className="spread book-track focused-book-track" style={{width:focusTrackWidth,flexDirection:visualReverse?'row-reverse':'row',transform:`translateX(${focusedShift}px)`}}>
              <div className={`active-page book-page book-page-${activeSide}`}><EditorCanvas page={page} width={pageCanvasWidth} onTextEdit={()=>setPanel('text')} onCrop={openCrop}/></div>
