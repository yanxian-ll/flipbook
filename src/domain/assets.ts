...
export async function prepareAsset(file:File):Promise<StoredAsset>{
  ...
  return {id,name:file.name,mimeType:file.type,width:image.naturalWidth,height:image.naturalHeight,orientation:ratio>1.2?'landscape':ratio<0.8?'portrait':'square',storageKey:id,createdAt:Date.now(),original:file,preview:await resize(image,1800),thumbnail:await resize(image,240)};
}
