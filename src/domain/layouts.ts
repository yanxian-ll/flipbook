...
export function layoutsForCount(count:number,theme:ThemeId):Layout[]{
  ...
  return layouts.filter(l=>!hidden.has(l.id)&&l.minImages===count&&(!l.family||l.family===family)).sort((a,b)=>{
    if(count===6&&(a.id==='six6Sidebar'||b.id==='six6Sidebar'))return a.id==='six6Sidebar'?-1:1;
    return Number(b.family===family)-Number(a.family===family);
  });
}
...
export function applyLayout(page:Page,layout:Layout,assetIds?:string[]):Page {
  ...
  const ids=assetIds??oldImages.map(e=>e.assetId!);
  if(ids.length!==layout.slots.length)throw new Error(`这个模板需要 ${layout.slots.length} 张照片，当前已选 ${ids.length} 张。`);
  const images=ids.map((id,i)=>{const slot=layout.slots[i];return imageElement(id,{id:oldImages[i]?.id??crypto.randomUUID(),x:slot.x*W,y:slot.y*H,width:slot.width*W,height:slot.height*H,frameLocked:true,frameShape:slot.shape});});
