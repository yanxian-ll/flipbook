...
function AssetTile({asset,selectionIndex,onClick,draggable=false,dragging=false,dragOver=false,onDragStart,onDragEnter,onDragOver,onDrop,onDragEnd}:{asset:Asset;selectionIndex:number;onClick:()=>void;draggable?:boolean;dragging?:boolean;dragOver?:boolean;onDragStart?:(e:React.DragEvent<HTMLButtonElement>)=>void;onDragEnter?:(e:React.DragEvent<HTMLButtonElement>)=>void;onDragOver?:(e:React.DragEvent<HTMLButtonElement>)=>void;onDrop?:(e:React.DragEvent<HTMLButtonElement>)=>void;onDragEnd?:()=>void}){const [url,setUrl]=useState('');useEffect(()=>{let url='',active=true;void repository.getAsset(asset.id).then(a=>{if(a&&active){url=URL.createObjectURL(a.thumbnail);setUrl(url);}}).catch(()=>{});return()=>{active=false;if(url)URL.revokeObjectURL(url);};},[asset.id]);return <button draggable={draggable} className={`${selectionIndex?'asset-tile-selected':''} ${dragging?'asset-tile-dragging':''} ${dragOver?'asset-tile-drag-over':''}`.trim()} title={selectionIndex?`已选第 ${selectionIndex} 张 · 可拖动排序 · ${asset.name}`:asset.name} aria-pressed={selectionIndex>0} onDragStart={onDragStart} onDragEnter={onDragEnter} onDragOver={onDragOver} onDrop={onDrop} onDragEnd={onDragEnd} onClick={onClick}><img src={url} alt={asset.name}/>{selectionIndex>0&&<span className="asset-selection-index" aria-label={`第 ${selectionIndex} 张`}>{selectionIndex}</span>}</button>;}
