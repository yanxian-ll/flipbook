...
export function EditorPanel({panel,onClose,onPanel,placement,photoIds:controlledPhotoIds,onPhotoIdsChange,paired=false}:EditorPanelProps){
  ...
  const [error,setError]=useState(''),[busy,setBusy]=useState(false),[count,setCount]=useState(0),[localPhotoIds,setLocalPhotoIds]=useState<string[]>(initialPhotoIds);const input=useRef<HTMLInputElement>(null);
...
function AssetTile({asset,selectionIndex,onClick}:{asset:Asset;selectionIndex:number;onClick:()=>void}){const [url,setUrl]=useState('');useEffect(()=>{let url='',active=true;void repository.getAsset(asset.id).then(a=>{if(a&&active){url=URL.createObjectURL(a.thumbnail);setUrl(url);}}).catch(()=>{});return()=>{active=false;if(url)URL.revokeObjectURL(url);};},[asset.id]);return <button className={selectionIndex?'asset-tile-selected':''} title={selectionIndex?`已选第 ${selectionIndex} 张 · ${asset.name}`:asset.name} aria-pressed={selectionIndex>0} onClick={onClick}><img src={url} alt={asset.name}/>{selectionIndex>0&&<span className="asset-selection-index" aria-label={`第 ${selectionIndex} 张`}>{selectionIndex}</span>}</button>;}
