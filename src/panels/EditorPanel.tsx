...
export function EditorPanel({panel,onClose,onPanel,placement,photoIds:controlledPhotoIds,onPhotoIdsChange,paired=false}:EditorPanelProps){
  ...
  const addText=(text:string,size:number,font?:string)=>{const style=book.bookStyle??{fontFamily:'Domine',textColor:'#252525'};s.addElement(textElement(text,{fontSize:size,fontFamily:font??style.fontFamily??'Domine',color:style.textColor??'#252525'}));};
