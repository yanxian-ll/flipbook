...
export function EditorPanel({panel,onClose,onPanel,placement,photoIds:controlledPhotoIds,onPhotoIdsChange,paired=false}:EditorPanelProps){
  ...
  </div><Modal wide open={customOpen} onClose={()=>setCustomOpen(false)} title="修改模板" description="直接拖动和缩放图框，保存为自己的模板"><label className="field stack">模板名称<input value={customName} onChange={e=>setCustomName(e.target.value)}/></label><VisualTemplateEditor slots={customSlots} onChange={setCustomSlots} assetIds={pagePhotoIds} background={page.templateBackground??page.background} overlay={page.templateOverlay} texts={page.elements.filter(e=>e.type==='text')}/>
<ErrorMessage message={error}/><Button className="primary full" onClick={()=>{if(customSlots.some(s=>s.width<=0||s.height<=0||s.x<0||s.y<0||s.x+s.width>1.001||s.y+s.height>1.001)){setError('请将所有图框放在页面范围内。');return;}const layout={id:'custom-'+uid(),name:customName.trim()||'我的模板',minImages:customSlots.length,maxImages:customSlots.length,slots:customSlots,background:page.templateBackground??page.background,overlay:page.templateOverlay,texts:page.elements.filter(e=>e.type==='text').map(e=>({x:e.x/1200,y:e.y/1696,width:e.width/1200,height:e.height/1696,key:e.templateTextKey??e.id,text:e.text??'',fontFamily:e.fontFamily??'Domine',fontSize:(e.fontSize??60)/1200,fontWeight:e.fontWeight??400,color:e.color??'#222',align:e.align??'left',lineHeight:e.lineHeight??1.2,letterSpacing:(e.letterSpacing??0)*320/1200}))};s.change(b=>{(b.customLayouts??=[]).push(layout);b.pages[s.pageIndex]=applyLayout(b.pages[s.pageIndex],layout);});setError('');setCustomOpen(false);}}>保存并应用模板</Button></Modal></aside>;
}
