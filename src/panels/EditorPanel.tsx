...
export function EditorPanel({panel,onClose,onPanel,placement,photoIds:controlledPhotoIds,onPhotoIdsChange,paired=false}:EditorPanelProps){
  ...
  const [error,setError]=useState(''),[busy,setBusy]=useState(false),[count,setCount]=useState(0),[localPhotoIds,setLocalPhotoIds]=useState<string[]>(initialPhotoIds),[dragAssetId,setDragAssetId]=useState<string|null>(null),[dragOverAssetId,setDragOverAssetId]=useState<string|null>(null),[assetFilter,setAssetFilter]=useState<'all'|'used'|'unused'>('all'),[assetSort,setAssetSort]=useState<'recent'|'oldest'|'name'>('recent'),[assetQuery,setAssetQuery]=useState('');const input=useRef<HTMLInputElement>(null),suppressAssetClick=useRef(false);
  ...
  function saveCustomTemplate(){
    ...
    if(page.type==='cover'){
      ...
      const layout={id:'custom-'+uid(),name:customName.trim()||'我的模板',minImages:customSlots.length,maxImages:customSlots.length,slots:customSlots,background:page.templateBackground??page.background,overlay:page.templateOverlay,texts:page.elements.filter(e=>e.type==='text').map(e=>({x:e.x/W,y:e.y/H,width:e.width/W,height:e.height/H,key:e.templateTextKey??e.id,text:e.text??'',fontFamily:e.fontFamily??'Domine',fontSize:(e.fontSize??60)/W,fontWeight:e.fontWeight??400,fontStyle:e.fontStyle,color:e.color??'#222',align:e.align??'left',lineHeight:e.lineHeight??1.2,letterSpacing:(e.letterSpacing??0)*320/W}))};
  ...
  return <aside className={`editor-panel ${placement?`panel-${placement}`:''} ${paired?'paired-library-panel':''}`}><div className="panel-grabber"/><header><h2>{page.type==='cover'&&panel==='layouts'?'封面模板':names[panel]}</h2><IconButton label="关闭面板" onClick={onClose}><X size={17}/></IconButton></header><ErrorMessage message={error}/><div className="panel-body">
    {panel==='photos'&&<><p className="muted">{page.type==='cover'?'点击任意素材，直接替换封面照片。':selected?.type==='image'?'点击任意素材，直接替换当前图框。':'选择本页照片（最多 9 张）'}</p><input ref={input} type="file" multiple accept="image/jpeg,image/png,image/webp,image/gif,image/avif" hidden onChange={e=>void upload(e.target.files)}/><div className="asset-library-manager"><div className="asset-library-tabs" role="tablist" aria-label="筛选素材"><button type="button" role="tab" aria-selected={assetFilter==='all'} className={assetFilter==='all'?'active':''} onClick={()=>setAssetFilter('all')}>全部 <small>{book.assets.length}</small></button><button type="button" role="tab" aria-selected={assetFilter==='used'} className={assetFilter==='used'?'active':''} onClick={()=>setAssetFilter('used')}>已使用 <small>{assetUsage.used}</small></button><button type="button" role="tab" aria-selected={assetFilter==='unused'} className={assetFilter==='unused'?'active':''} onClick={()=>setAssetFilter('unused')}>未使用 <small>{assetUsage.unused}</small></button></div><div className="asset-library-tools"><input type="search" value={assetQuery} onChange={e=>setAssetQuery(e.target.value)} placeholder="搜索文件名" aria-label="搜索素材"/><select value={assetSort} onChange={e=>setAssetSort(e.target.value as 'recent'|'oldest'|'name')} aria-label="素材排序"><option value="recent">最近上传</option><option value="oldest">最早上传</option><option value="name">文件名 A–Z</option></select></div></div><div className="asset-batches">{selectedAssets.length>0&&<section className="asset-batch asset-batch-selected"><div className="asset-batch-header"><span><b>本页已选</b></span><small>{selectedAssets.length} 张</small></div><div className="asset-grid">{selectedAssets.map((asset,index)=><AssetTile key={asset.id} asset={asset} selectionIndex={index+1} draggable dragging={dragAssetId===asset.id} dragOver={dragOverAssetId===asset.id&&dragAssetId!==asset.id} onDragStart={e=>{suppressAssetClick.current=true;setDragAssetId(asset.id);setDragOverAssetId(asset.id);e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',asset.id);}} onDragEnter={e=>{e.preventDefault();if(dragAssetId&&dragAssetId!==asset.id)setDragOverAssetId(asset.id);}} onDragOver={e=>{e.preventDefault();e.dataTransfer.dropEffect='move';}} onDrop={e=>{e.preventDefault();reorderSelectedAsset(asset.id);setDragAssetId(null);setDragOverAssetId(null);window.setTimeout(()=>{suppressAssetClick.current=false;},0);}} onDragEnd={()=>{setDragAssetId(null);setDragOverAssetId(null);window.setTimeout(()=>{suppressAssetClick.current=false;},0);}} onClick={()=>{setError('');chooseAsset(asset.id);}}/>)}</div></section>}<VirtualAssetLibrary batches={assetBatches} label={(batch,index)=>assetSort==='name'?<>按文件名 <b>A–Z</b></>:<>{batch.legacy?'较早上传':assetSort==='oldest'?(index===0?'最早上传':'上传于'):(index===0?'最近上传':'上传于')} <b>{batch.legacy?'':formatBatch(batch)}</b></>} renderAsset={asset=><AssetTile key={asset.id} asset={asset} selectionIndex={0} used={usedAssetIds.has(asset.id)} onClick={()=>{setError('');chooseAsset(asset.id);}}/>}/>{book.assets.length>0&&libraryAssetCount===0&&<p className="asset-library-empty">{assetQuery?'没有匹配这个文件名的其他素材':'当前筛选下没有其他素材'}</p>}</div>{!book.assets.length&&<p className="empty-panel">还没有照片<br/>从下方添加照片开始制作。</p>}<div className="photo-selection-actions photo-library-actions"><Button disabled={busy} onClick={()=>input.current?.click()}><Upload size={15}/>{busy?`处理中 ${count}`:'添加照片'}</Button><Button className="danger" disabled={busy||!photoIds.length} onClick={()=>void removeSelectedAssets()}><Trash2 size={15}/>删除</Button><Button className="primary" disabled={busy} onClick={()=>{s.setPhotos(photoIds);if(!photoIds.length){if(!paired)onClose();return;}if(page.type==='cover')onClose();else if(!paired)onPanel('layouts');}}>使用 {photoIds.length} 张</Button></div></>}
    ...
    {panel==='text'&&<>{templateTexts.length>0&&<section className="template-text-editor-list"><p className="field-label">本页模板文字</p>{templateTexts.map((element,index)=><label key={element.id} className="template-text-editor-item"><span>{index+1}</span><textarea rows={Math.min(3,Math.max(1,(element.text??'').split('\n').length))} value={element.text??''} onFocus={()=>s.select(element.id)} onChange={e=>s.updateElement(element.id,{text:e.target.value})}/></label>)}</section>}{selected?.type==='text'?<><p className="field-label">{selected.templateTextKey?'当前模板文字':'当前文字'}</p><textarea aria-label="文字内容" value={selected.text} onChange={e=>update({text:e.target.value})} rows={3}/><label className="field">字体<select value={selected.fontFamily} onChange={e=>update({fontFamily:e.target.value})}>{selected.fontFamily&&!['Domine','Arial','Georgia','KaiTi','sans-serif','Mendl Sans Dusk','Mendl Sans Dusk Medium','FZLanTingHei'].includes(selected.fontFamily)&&<option value={selected.fontFamily}>{selected.fontFamily} · 模板字体</option>}<option value="Mendl Sans Dusk">Mendl Sans Dusk · 模板字体</option><option value="Mendl Sans Dusk Medium">Mendl Sans Dusk Medium · 模板中黑</option><option value="FZLanTingHei">方正兰亭黑 · 模板字体</option><option value="Domine">Domine · 杂志衬线</option><option value="Arial">Arial · 现代无衬线</option><option value="Georgia">Georgia · 经典</option><option value="KaiTi">楷体 · 手写</option><option value="sans-serif">黑体</option></select></label><label className="field">字号<input type="range" min={8} max={240} value={selected.fontSize} onChange={e=>update({fontSize:+e.target.value})}/><span>{Math.round(selected.fontSize??0)}</span></label><label className="field">颜色<input type="color" value={selected.color} onChange={e=>update({color:e.target.value})}/></label><div className="segments"><Button className={selected.fontWeight===700?'primary':''} onClick={()=>update({fontWeight:selected.fontWeight===700?400:700})}><b>B</b></Button><Button className={selected.fontStyle==='italic'?'primary':''} onClick={()=>update({fontStyle:selected.fontStyle==='italic'?'normal':'italic'})}><i>I</i></Button>{(['left','center','right'] as const).map((align,i)=><Button key={align} className={selected.align===align?'primary':''} onClick={()=>update({align})}>{['左','中','右'][i]}</Button>)}</div></>:templateTexts.length===0?<p className="muted">选择一段文字，或者添加新的文字</p>:null}<div className="text-presets"><button onClick={()=>addText('写下这一刻',100)}>添加标题 <Plus size={16}/></button><button onClick={()=>addText('一些值得记住的小事',54)}>添加副标题 <Plus size={16}/></button><button onClick={()=>addText('你的段落文字',36)}>添加正文 <Plus size={16}/></button><button onClick={()=>addText(new Date().toLocaleDateString('zh-CN'),28)}>日期 / 注释 <Plus size={16}/></button></div></>}
