...
export function IconButton({label,children,active=false,...props}:ButtonHTMLAttributes<HTMLButtonElement>&{label:string;active?:boolean}){return <button title={label} aria-label={label} className={`icon-btn ${active?'active':''}`} {...props}>{children}</button>;}
export function Modal({open,onClose,title,description,children,wide=false}:{open:boolean;onClose:()=>void;title:string;description?:string;children:ReactNode;wide?:boolean}){return <Dialog.Root open={open} onOpenChange={v=>{if(!v)onClose();}}><Dialog.Portal><Dialog.Overlay className="modal-shade"/><Dialog.Content className={`modal ${wide?'wide':''}`} aria-describedby={description?'modal-description':undefined}><Dialog.Close className="modal-close" aria-label="关闭"><X size={16}/></Dialog.Close><Dialog.Title>{title}</Dialog.Title>{description&&<Dialog.Description id="modal-description">{description}</Dialog.Description>}{children}</Dialog.Content></Dialog.Portal></Dialog.Root>;}
