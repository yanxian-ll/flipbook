...
export function PageThumbnail({page,alt='页面预览',scale=.12,immediate=false}:{page:Page;alt?:string;scale?:number;immediate?:boolean}){
  const [url,setUrl]=useState(''),[visible,setVisible]=useState(false);const host=useRef<HTMLDivElement>(null);
  useEffect(()=>{const node=host.current;if(!node)return;const observer=new IntersectionObserver(entries=>setVisible(entries[0].isIntersecting),{rootMargin:'120px'});observer.observe(node);return()=>observer.disconnect();},[]);
  useLayoutEffect(()=>{if(!visible){setUrl('');return;}let alive=true,objectUrl='',timer:ReturnType<typeof setTimeout>|undefined;
